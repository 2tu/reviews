...
import com.tu.review.data.source.LeanCloudRepository;
...
public final class MyAppPresenter extends RxPresenter<MyAppView> {
  ...
  private final LeanCloudRepository leanCloudRepository;

  @Inject
  public MyAppPresenter(MyAppRepository repository, LeanCloudRepository leanCloudRepository) {
    this.repository = repository;
    this.leanCloudRepository = leanCloudRepository;
  }
  ...
  public void load(final String packageName) {
    ...
    subscriptions.add(observeOnView(
        repository.detailByLeanCloud(packageName).map(new Func1<ResponseBody, AppInfo>() {
          @Override public AppInfo call(ResponseBody responseBody) {
            if (MediaType.parse("application/json; charset=utf-8")
                .equals(responseBody.contentType())) {
      ...
      @Override public void onEnd() {
        ...
        subscriptions.add(observeOnView(
            repository.detailByMyApp(packageName).map(new Func1<ResponseBody, AppInfo>() {
              @Override public AppInfo call(ResponseBody responseBody) {
                return parse(responseBody, packageName);
              }
            })).subscribe(new EndSubscriber<AppInfo>() {
          @Override public void onEnd() {
          }
          ...
          @Override public void onNext(AppInfo appInfo) {
            if (null == appInfo) {
              ...
              leanCloudRepository.save(appInfo);
            }
          }
        }));
      }
  ...
  public AppInfo parse(ResponseBody responseBody, String packageName) {
